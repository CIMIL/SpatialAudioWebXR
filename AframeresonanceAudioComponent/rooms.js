//? Because the CDN of this component doesn't work I imported the source code directly
...
AFRAME.registerSystem("building", {
  reexamineBuilding: function () {
    ...
    function makeUvsForGeom(geom, callback) {
      ...
      for (
        var faceVertIndex = 0;
        faceVertIndex < geom.index.array.length;
        faceVertIndex++
      ) {
    ...
    function makePlaneUvs(geom, uKey, vKey, uMult, vMult) {
      makeUvsForGeom(geom, function (pt) {
        return [pt[uKey] * uMult, pt[vKey] * vMult];
      });
    }
    ...
    function getUnsortedRoomWallArray(roomEl) {
      var walls = [];
      for (
        var roomChildNodeIndex = 0;
        roomChildNodeIndex < roomEl.children.length;
        roomChildNodeIndex++
      ) {
    ...
    function getRoomWallArray(roomEl) {
      ...
      for (var wallIndex = 0; wallIndex < walls.length; wallIndex++) {
        ...
        cwSum +=
          (nextWallPos.x - curWallPos.x) * (nextWallPos.z + curWallPos.z);
      }
    ...
    function moveForLink(doorholeEl, doorlinkEl) {
      ...
      var localLinkX =
        linkGapX * Math.cos(-wallAng) - linkGapZ * Math.sin(-wallAng);
    ...
    function getWallHeight(wallEl) {
      if (wallEl.components.wall.data.height)
        return wallEl.components.wall.data.height;
    ...
    setTimeout(function () {
      ...
      for (
        var sceneChildNodeIndex = 0;
        sceneChildNodeIndex < buildingSelf.el.children.length;
        sceneChildNodeIndex++
      ) {
        ...
        if (sceneChildNode.components && sceneChildNode.components.room) {
          ...
          if (walls.length > 2) {
            for (var wallIndex = 0; wallIndex < walls.length; wallIndex++) {
              ...
              var wallGapX =
                nextWallNode.components.position.data.x -
                curWallNode.components.position.data.x;
              var wallGapZ =
                nextWallNode.components.position.data.z -
                curWallNode.components.position.data.z;
              var wallAng = Math.atan2(wallGapZ, wallGapX);

              curWallNode.setAttribute("rotation", {
                x: 0,
                y: (-wallAng / Math.PI) * 180,
                z: 0,
              });
      ...
      var doorlinks = buildingSelf.el.querySelectorAll("[doorlink]");
      for (
        var curDoorlinkElIndex = 0;
        curDoorlinkElIndex < doorlinks.length;
        curDoorlinkElIndex++
      ) {
        ...
        if (sceneChildNode.components && sceneChildNode.components.room) {
          ...
          if (walls.length > 2) {
            for (var wallIndex = 0; wallIndex < walls.length; wallIndex++) {
              ...
              var heightGap =
                getWallHeight(nextWallNode) - getWallHeight(curWallNode);

              var orderedHoles = [];
              for (
                var wallChildNodeIndex = 0;
                wallChildNodeIndex < curWallNode.children.length;
                wallChildNodeIndex++
              ) {
                var wallChildNode = curWallNode.children[wallChildNodeIndex];
                if (
                  wallChildNode.components &&
                  wallChildNode.components.doorhole
                ) {
              ...
              orderedHoles.sort(function (a, b) {
                return (
                  a.components.position.data.x - b.components.position.data.x
                );
              });
              ...
              for (
                ...
                for (var holeSide = -1; holeSide <= 1; holeSide += 2) {
                  var ptX =
                    holeInfo.position.data.x +
                    (linkInfo.doorlink.data.width / 2) * holeSide;
                  var floorY = (ptX / wallLength) * wallGapY;
                  var topY = floorY + linkInfo.doorlink.data.height;

                  var curCeil =
                    getWallHeight(curWallNode) + (ptX / wallLength) * heightGap;
              ...
              finishGeom(wallGeom);
              var myMat = curWallNode.components.material
                ? curWallNode.components.material.material
                : curWallNode.parentNode.components.material.material;
            ...
            for (
              var roomChildNodeIndex = 0;
              roomChildNodeIndex < sceneChildNode.children.length;
              roomChildNodeIndex++
            ) {
              var roomChildNode = sceneChildNode.children[roomChildNodeIndex];
              if (roomChildNode.components) {
                if (
                  roomChildNode.components.floor ||
                  roomChildNode.components.ceiling
                )
                  caps.push(roomChildNode);
              }
            }
            for (
              var curCapNodeIndex = 0;
              curCapNodeIndex < caps.length;
              curCapNodeIndex++
            ) {
              ...
              var myMat = curCapNode.components.material
                ? curCapNode.components.material.material
                : curCapNode.parentNode.components.material.material;
      ...
      for (
        ...
        for (
          var doorLinkChildIndex = 0;
          doorLinkChildIndex < curDoorlinkEl.children.length;
          doorLinkChildIndex++
        ) {
          ...
          for (
            ...
            var myMat = doorLinkChild.components.material
              ? doorLinkChild.components.material.material
              : doorLinkChild.parentNode.components.material.material;
            ...
            if (!doorLinkChild.myGeoms[curType]) {
              ...
              var myMesh = new THREE.Mesh(curGeom, myMat);
            ...
            switch (curType) {
              case "floor":
                ...
                makeUvsForGeom(curGeom, function (pt, vertIndex) {
                  return [1 - (vertIndex % 2), 1 - Math.floor(vertIndex / 2)];
                });
              ...
              case "ceiling":
                ...
                makeUvsForGeom(curGeom, function (pt, vertIndex) {
                  return [vertIndex % 2, 1 - Math.floor(vertIndex / 2)];
                });
...
function updateScene(lastScene) {
  if (lastScene && lastScene.systems.building)
    lastScene.systems.building.reexamineBuilding();
}
